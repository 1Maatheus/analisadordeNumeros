...
function adicionar() {
  if (isNumero(num.value) && !inLista(num.value, valores)) {
    ...
    item.text = `Valor ${num.value} adicinonado`;
...
function finalizar() {
  if (valores.length == 0) {
    ...
    res.innerHTML += `<p>A soma dos valores informado foi de <strong>${soma}</strong>.</p>`;
    res.innerHTML += `<p>A media dos valores é de <strong>${media}</strong>.</p>`;
  }
}
